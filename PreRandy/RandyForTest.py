...
def newCoupon(cityId,agencyId,server_url,url_username,url_password,coupon_value,coupon_quantity,coupon_useBaseLine,coupon_instruction):
    ...
    conn_test.commit()
    cur.close()
    print('是否绑定优惠券到店铺(输入数字选择):'.encode('gb18030'))
    isUse = input('1.绑定店铺,\n2.不绑定店铺,\n'.encode('gb18030'))
    if str(isUse)==str(1):
        store_main_username = input('输入店铺登陆账号:'.encode('gb18030'))
        store_main_password = input('输入店铺登陆密码(默认123456,可回车跳过):'.encode('gb18030'))
...
def create_san_product(name,img_jpg):
    ...
    try:
        conn=MySQLdb.connect(host=test_db_ip, user=test_user, passwd=test_passwd, port=3306, charset="utf8")
        cur = conn.cursor()
...
def ThOrder(cityId,agencyId,service_url,service_username,service_password,orderId,invent_url,invent_username,mainhouse):
    ...
    newSession.post(url=service_changecity_url,data={'cityId':cityId,'agencyId':agencyId})
    #点单明细
    conn_test_1 = MySQLdb.connect(host=test_db_ip, user=test_user, passwd=test_passwd, port=3306, charset="utf8")
...
def Ruku(cityId,agencyId,worker,orderId,invent_url,url_username,url_password):
    ...
    conn_test11.close()
    time.sleep(2)
    conn_test111 = MySQLdb.connect(host=test_db_ip, user=test_user, passwd=test_passwd, port=3306, charset="utf8")
...
def main():
    ...
    while True:
        print()
        print('*'*30)
        print('请选择以下业务(输入数字选择):'.encode('gb18030'))
        ...
        if str(order_num)==str(1):
            print('请选择城市(输入数字选择):'.encode('gb18030'))
            city_num=input('\n1.南京,\n\n2.常州,\n\n3.无锡,\n\n4.连云港,\n'.encode('gb18030'))
            if str(city_num)==str(1):
                ...
                continue
            redGift_value = input('红包金额:'.encode('gb18030'))
            redGift_quantity = input('红包发放数量:'.encode('gb18030'))
            redGift_ownLimit = input('用户可领取数量:'.encode('gb18030'))
            redGift_useBaseLine = input('红包使用条件金额(满多少可用?):'.encode('gb18030'))
            ...
            print('请选择是否店铺领取(输入数字选择):'.encode('gb18030'))
            bangding_num=input('\n1.领取,\n2.不领取,\n'.encode('gb18030'))
            ...
            if str(city_num)==str(1):
                ...
                continue
            coupon_value = input('优惠券金额:'.encode('gb18030'))
            coupon_quantity = input('优惠券发放数量:'.encode('gb18030'))
            coupon_useBaseLine = input('优惠券使用条件金额(满多少可用?):'.encode('gb18030'))
            ...
            dd_phoneNum = input('输入店铺登陆账号:'.encode('gb18030'))
            dd_count = input('输入达豆数量:'.encode('gb18030'))
            ...
            zxx_loginNum = input('输入猪行侠登陆账号:'.encode('gb18030'))
            ...
            if str(task_num)==str(1):
                ...
                store_name = input('输入店铺登录账号:'.encode('gb18030'))
