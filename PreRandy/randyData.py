...
class preConditionForTest(object):
    ...
    def __init__(self):
        ...
        self.redGift_instruction='满40减10'
        """
        coupon_value:优惠券金额
    ...
    def createNewRedGift(self):
        ...
        lifeTime:使用周期不限制
        """
        redgift_data = {'name': name, 'type': '普通红包', 'value': self.redGift_value, 'grantStart': grantStart,
        ...
        try:
            for cj in check_json['stores'][0]['orders']:
                if cj['state']==0:
                    ...
                    cancle_response=newSession.post(url=cancle_url,data={'id':int(cj['id']),'info':'客户下错单'})
